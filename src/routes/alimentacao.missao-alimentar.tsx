...
export const Route = createFileRoute("/alimentacao/missao-alimentar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Um pequeno gesto alimentar para hoje." },
    ],
  }),
  ...
  return (
    <AppShell title="Missão alimentar" subtitle={`Dia ${dayId}`} back="/alimentacao">
      <section className="rounded-2xl border border-primary/20 bg-warm p-5">
        ...
        >
          {done ? <><Undo2 size={16} aria-hidden /> Desfazer</> : <><Check size={16} aria-hidden /> Marcar como feita</>}
        </button>
      </section>

      <p className="mt-6 text-xs text-text-muted">
        A missão definitiva do dia será inserida em uma etapa posterior.
      </p>
